...
import 'package:bizzcoin_app/Screen/HomePage/ItemByGroup.dart';
...
class Grouped extends StatefulWidget {
  ...
  const Grouped({Key key, this.id, this.name}) : super(key: key);
  @override
  _GroupedState createState() => _GroupedState(this.id, this.name);
}
...
class _GroupedState extends State<Grouped> {
  ...
  _GroupedState(this.id, this.name);
  ...
  void onGroupedPress() async {
    ...
    String url = RouteApi().routeGet(name: "grouped") + id.toString();
      ...
      category.forEach((element) {
        if (lang == 'kus') {
          grouped.add(
            GroupedItem(
              image: NetworkImage(
                  "${RouteApi().storageUrl}${element['gr_image']}"),
              name: element['gr_name_ku'],
              onItemPressed: () =>
                  onItemGroup(element['id'], element['gr_name_ku']),
            ),
          );
        } else if (lang == 'en') {
          grouped.add(
            GroupedItem(
              ...
              onItemPressed: () =>
                  onItemGroup(element['id'], element['gr_name']),
            ),
          );
        } else if (lang == 'ar') {
          grouped.add(
            GroupedItem(
              image: NetworkImage(
                  "${RouteApi().storageUrl}${element['gr_image']}"),
              name: element['gr_name_ar'],
              onItemPressed: () =>
                  onItemGroup(element['id'], element['gr_name_ar']),
            ),
          );
        } else if (lang == 'kuk') {
          grouped.add(
            GroupedItem(
              image: NetworkImage(
                  "${RouteApi().storageUrl}${element['gr_image']}"),
              name: element['gr_name_kr'],
              onItemPressed: () =>
                  onItemGroup(element['id'], element['gr_name_kr']),
            ),
          );
        } else if (lang == 'per') {
          grouped.add(
            GroupedItem(
              image: NetworkImage(
                  "${RouteApi().storageUrl}${element['gr_image']}"),
              name: element['gr_name_pr'],
              onItemPressed: () =>
                  onItemGroup(element['id'], element['gr_name_pr']),
            ),
          );
        }
      });
  ...
  void onItemGroup(id, name) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ItemByGrouped(id: id, name: name),
      ),
    );
  }
}
